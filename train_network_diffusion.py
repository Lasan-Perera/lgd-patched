...
def validate(net, diffusion, schedule_sampler, device, val_data, iou_threshold):
    ...
    with torch.no_grad():
        for i, (x, y, didx, rot, zoom_factor, prompt, query) in enumerate(val_data):
            if i >= 3:
                break
            img = x.to(device)
...
def run():
    ...
    # Two independent datasets, not one index-sliced set -- the original
    # code filtered BOTH train and val against the TEST split (see
    # notes/paper_vs_code.md #4). Train now draws from the real 14,516-
    # sample train/seen.obj; validation keeps the original test-split
    # behaviour (args.seen picks test/seen.obj vs test/unseen.obj) so it
    # stays comparable to the paper's seen/unseen protocol.
    train_dataset = Dataset(args.dataset_path,
                      ...
                      split_file='split/grasp-anything++/train/seen.obj')
    ...
    diffusion.contrastive_loss_type = 'eq4'
