...
class LGDM(LanguageGraspModel):
    ...
    def _init_albef(self):
        ...
        # --- load pretrained RefCOCO+ grounding checkpoint ---
        from inference.models.lgdm.albef.models.vit import interpolate_pos_embed
        ckpt_path = 'checkpoints/refcoco.pth'
        checkpoint = torch.load(ckpt_path, map_location='cpu')
        state_dict = checkpoint.get('model', checkpoint)
        state_dict['visual_encoder.pos_embed'] = interpolate_pos_embed(
            state_dict['visual_encoder.pos_embed'], self.albef.visual_encoder)
        if 'visual_encoder_m.pos_embed' in state_dict:
            state_dict['visual_encoder_m.pos_embed'] = interpolate_pos_embed(
                state_dict['visual_encoder_m.pos_embed'], self.albef.visual_encoder_m)
        missing, unexpected = self.albef.load_state_dict(state_dict, strict=False)
        print(f"[ALBEF ckpt] missing={len(missing)} unexpected={len(unexpected)}")
    ...
    def forward(self, x, img, t, query, alpha, idx, prompt=None):
        ...
        # Combine textual features with the visual features (restored --
        # released code left this disabled, so y_flatten never received a
        # gradient. .detach() also removed so gradients reach conv1-3 too,
        # unlike the original commented line -- see notes/paper_vs_code.md)
        img = img + y
